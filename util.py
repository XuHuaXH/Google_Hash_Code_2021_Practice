...
def write(solution, i):
    with open("./sol" + str(i), "w") as outfile:
...
def write_minimal(solution, pizza_data, i):
    with open("./sol" + str(i), "w") as outfile:
        outfile.write(str(len(solution)) + '\n')
        for delivery in solution:
            line = str(len(delivery)) + ' '
            for index in delivery:
                ...
                line += ' '
            outfile.write(line + '\n')
    print('The score for case ' + str(i) + ' is ' + str(score_minimal_solver(solution, pizza_data)))
